...
check_body() {
  ...
  if [[ "$body" == *"$needle"* ]]; then
    ...
    # читателя проверять файл, который в порядке. Именно эта улика и раскрыла SIGPIPE — тело было
    # 43638 байт и начиналось с искомой строки, а проверка говорила «не содержит».
