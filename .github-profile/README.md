...
Восемь самостоятельных проектов, которые складываются друг на друга. Внизу — типизированный SDK, всё остальное стоит на нём. Бери один кубик или весь стенд.
...
flowchart TD
    ...
    lztflows["lzt-flows · каталог модулей"]
    lztui["lzt-ui · UI-кит панели"]
    ...
    lztflows -. каталог для .-> autolzt
    lztui -. UI для .-> autolzt
    ...
    class pylzt base
    class eventus,autolzt,mcp,eventussdk,testnet,lztflows,lztui node
```
...
| **[auto-lzt](https://github.com/open-lzt/auto-lzt)** (публикуется также как **lzt-flow**) | Серверный движок **no-code автоматизаций**. Описываешь задачу («поднимай лоты каждый час») как граф-флоу — движок исполняет. Расширяется плагинами. | [дизайн флоу](https://github.com/open-lzt/auto-lzt/blob/main/docs/flow-design-guide.md) · [плагины](https://github.com/open-lzt/auto-lzt/blob/main/docs/plugins.md) |
| **[lzt-flows](https://github.com/open-lzt/lzt-flows)** | Каталог готовых модулей-флоу для auto-lzt — ставятся `install-flow.sh` одной командой. | [список модулей](https://github.com/open-lzt/lzt-flows#readme) |
...
| **[lzt-ui](https://github.com/open-lzt/lzt-ui)** | UI-кит веб-панели стенда: токены дизайна, компоненты, демо-форум. | [README](https://github.com/open-lzt/lzt-ui#readme) |
...
Одна команда поднимает стенд и прогоняет по нему все проекты, показывая каждый запрос и каждый ответ.

Идёт в **testnet-режиме**: запросы уходят во встроенный мок, реальный маркет не задет, токен не нужен.

---

### Запуск за одну команду

Монорепо **[open-lzt](https://github.com/open-lzt/open-lzt)** собирает все проекты в единый `systemd`-стенд на одном Linux-хосте:
...
Стенд уже стоит — готовый флоу ставится так же, одной командой:
