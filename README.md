...
| Проект | Роль | Локальные доки |
|---|---|---|
| [pylzt](projects/pylzt) | Типизированный async-SDK над API маркета — фундамент | [README](projects/pylzt/README.md) |
| [testnet](projects/testnet) | Мок lzt.market — оффлайн-двойник, в который ходят все сервисы | [docs/for_ai](projects/testnet/docs/for_ai/) |
| [eventus](projects/eventus) | Движок событий: опрос → долговечный лог → REST/webhook/SSE/WS | [architecture](projects/eventus/docs/architecture.md) · [extending](projects/eventus/docs/extending.md) |
| [eventus-sdk](projects/eventus-sdk) | Async-клиент к eventus | [architecture](projects/eventus-sdk/docs/architecture.md) |
| [flow](projects/flow) | No-code движок автоматизаций (флоу + плагины) | [flow-design](projects/flow/docs/flow-design-guide.md) · [modules](projects/flow/docs/modules.md) · [plugins](projects/flow/docs/plugins.md) |
| [mcp](projects/mcp) | MCP-сервер для ИИ-агентов (testnet по умолчанию) | [README](projects/mcp/README.md) |
| [lzt-ui](projects/lzt-ui) | UI-кит в стиле LZT: токены, компоненты, демо-форум | [README](projects/lzt-ui/README.md) |

[Доки для ИИ](docs/for_ai/) · [Архитектура](docs/ARCHITECTURE.md) · [Зачем](docs/WHY.md) · [Контрибуция](CONTRIBUTING.md)
...
По умолчанию стенд только loopback (достучись через SSH-туннель ниже). Во время `install.sh` он
спрашивает, выставлять ли его по HTTPS:

- **Введи домен** → он ставит nginx + сертификат **Let's Encrypt** (`certbot`, авто-обновление) и
  реверс-проксирует `https://<domain>/` → flow API, `https://<domain>/eventus/` → eventus. DNS домена
  уже должен указывать на сервер, а порты 80/443 быть открыты.
- **Без домена** → он предложит **самоподписанный сертификат на IP сервера** (браузеры покажут
  предупреждение).
- **Ни то, ни другое** → останется только loopback.
