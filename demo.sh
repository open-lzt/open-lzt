...
# ── панель ─────────────────────────────────────────────────────────────────────
# Последней сценой, а не первой: всё выше показывает работу через API, и человеку логично увидеть
# сайт уже после того, как он понял, что под ним крутится.
step "Панель — собирается и разворачивается"
say "панель в бете: часть экранов может не работать или отваливаться — API под ней стабилен"

HOST_IP=$(hostname -I 2>/dev/null | awk '{print $1}')
HOST_IP="${HOST_IP:-127.0.0.1}"

# Установщик и собирает фронтенд, и ставит nginx-сайт с сертификатом. Повторный запуск ничего не
# ломает: сертификат не перевыпускается, поэтому исключение в браузере переживает обновление.
say "разворачиваю сайт: install.sh соберёт панель и настроит nginx с сертификатом"
say "это повторный запуск установщика — он идемпотентен, сертификат не перевыпускается"
if bash "$ROOT/install.sh" --yes --market-mode "$MODE" </dev/null > /tmp/_panel_deploy.log 2>&1; then
  grep -aE 'панель|panel|сертификат' /tmp/_panel_deploy.log | tail -3
  ok "сайт развёрнут"
else
  fail "развёртывание сайта не прошло — хвост лога:"
  tail -5 /tmp/_panel_deploy.log
fi

# Панель отдаёт nginx, а не API — прошлая версия демо стучалась в $FLOW/panel/, которого не
# существует, печатала 404 и всё равно объявляла успех на последнем экране.
# `-k`: сертификат самоподписанный (публичный CA не выдаёт сертификат на голый IP), проверка
# доверия тут не то, что проверяется.
PANEL_URL="${PANEL_URL:-https://127.0.0.1/}"
panel_status=$(curl -sSk -o /tmp/_panel.html -w '%{http_code}' "$PANEL_URL" 2>/dev/null || echo 000)
...
if [[ "$panel_status" == "200" ]] && grep -qi '<html\|<!doctype' /tmp/_panel.html; then
  ...
  printf '\n  %s%sОткрой в браузере:%s  %shttps://%s/%s\n' \
    "$c_bold" "$c_green" "$c_reset" "$c_bold" "$HOST_IP" "$c_reset"
  say "сертификат самоподписанный — браузер предупредит один раз, это ожидаемо"
  say "панель пока в бете: если экран пуст или ломается — это она, а не стенд, всё то же есть в API"
else
