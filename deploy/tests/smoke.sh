...
check_body() {
  ...
  # Код возврата grep различается ЯВНО: 0 — нашёл, 1 — не нашёл, 2 — сам не смог. `if grep …`
  # сваливает 2 в ту же ветку, что и 1, и тогда упавший grep отчитывается как отсутствующее
  # содержимое. Найдено 17.08 по улике, которую печатает ветка ниже: тело было 43638 байт и
  # НАЧИНАЛОСЬ с искомой строки, а проверка говорила «не содержит». Причина — бокс на 1 vCPU и
  # 2 ГБ: рядом идёт сборка сайта, и очередной fork срывается. Отсюда и то, что отказ ловился
  # только внутри полного прогона и никогда в одиночку.
  printf '%s' "$body" | grep -qF -- "$needle"
  local found=$?
  if [ "$found" -eq 0 ]; then
    printf '%s ok %s %s contains %s\n' "$c_ok" "$c_off" "$path" "$needle"
  elif [ "$found" -gt 1 ]; then
    printf '%sFAIL%s %s: grep не смог проверить (код %s) — это отказ ПРОВЕРКИ, не сайта\n' \
      "$c_bad" "$c_off" "$path" "$found"
    fails=$((fails + 1))
  else
