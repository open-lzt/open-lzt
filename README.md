...
## Управлять автобаями из консоли

Панель — не единственный способ. `lzt-flow` делает то же самое из терминала, и это удобнее, когда
надо быстро проверить гипотезу или понять, почему прогон упал.

```bash
cd /opt/open-lzt/projects/flow && .venv/bin/lzt-flow status
```

`status` — режим маркета и состояние всех сервисов одной таблицей.

Поставить модуль и сразу задать параметры:

```bash
.venv/bin/lzt-flow install steam-autobuy --param max_price=10 --param count=1
```

Флоу создаётся **и компилируется** — сразу после этого его можно запускать.

```bash
.venv/bin/lzt-flow run <flow_id> --watch
```

**`run` не тратит деньги.** Если флоу объявляет `dry_run`, он принудительно ставится в `true`, и
команда говорит об этом до старта. Реальная покупка — только с явным `--no-dry-run`.

Разбор упавшего прогона — по шагам, с длительностью каждого узла:

```bash
.venv/bin/lzt-flow trace <flow_id>
```

Остальное: `modules` — что есть в каталоге, `list` — созданные флоу, `params` — объявленные
параметры флоу, `runs` — недавние прогоны, `accounts` — аккаунты и `accounts add --token …`.

Любой команде можно дать `--json` — вывод станет машиночитаемым. Ставится **до** подкоманды:
`lzt-flow --json list`, не `lzt-flow list --json`.
