...
export const ru = {
  meta: {
    ...
    description:
      // То же, что в герое, и по той же причине: это описание уезжает в превью ссылки, то
      // есть его читают ДО того, как сайт открыт. Названия сценариев тут важнее слоёв.
      'Поднятие лотов и тем, автобай по вашим условиям — открытый движок для lzt.market. Ставится на свой сервер одной командой, реальные деньги не тратит, пока сами не переключите.',
  },
  ...
  hero: {
    ...
    titleAccent: 'Открыто.',
    // Первым — что он делает, а не из чего собран. Слов «автобай», «поднятие лотов» и
    // «поднятие тем» на сайте не было ни одного, то есть человек, пришедший ровно за этим,
    // не находил их и уходил. Архитектура осталась, но второй строкой: за ней приходят те,
    // кто уже понял, зачем.
    // Три строки на телефоне, а не шесть: из чего он собран, читатель узнает ниже — там на это
    // есть целый раздел, и повторять его здесь значит платить шестистрочным абзацем на первом
    // экране за то, что и так будет сказано.
    sub: 'Поднимает лоты и темы, выкупает нужное по вашим условиям — без единой строки кода. Стенд встаёт на ваш сервер и остаётся вашим, реальных денег не тратит, пока сами не переключите.',
    ctaGithub: 'Смотреть на GitHub',
    pip: 'pip install pylzt',
    demo: {
      caption:
        'Главный путь: одна команда на чистом сервере — поставит стенд и покажет его по сценам. Ниже — только SDK, если нужен код, а не стенд.',
  ...
  builder: {
    ...
    fields: {
      ...
      yes: {
        label: 'Не задавать вопросов',
        hint: 'тогда токен бота и админов впишите в поля выше — иначе бот не стартует',
      },
  ...
  install: {
    ...
    req: 'Нужен сервер с Debian или Ubuntu, systemd и доступом root — подойдёт любой VPS от 300 ₽ в месяц. Советуем от 2 ГБ памяти: стенд поднимает Postgres, Redis и три сервиса. Docker скрипт поставит сам.',
    tabs: [
      {
        id: 'sdk',
        label: 'Только SDK',
  ...
  hosting: {
    ...
    paid: {
      ...
      per: 'в месяц',
      currency: { stars: 'звёзд', usd: '$', rub: '₽' },
      currencyLabel: 'Валюта',
      cta: 'Открыть бота',
      ctaNoPrice: 'Узнать цену в боте',
      priceNote: 'Один стенд, сценарии без лимита. Оплата в Telegram, отмена в любой момент.',
      noteNoPrice: 'Точная сумма и способ оплаты — в боте, до того как что-то нажато.',
      points: [
